...
export default function FeatureComponent(props) {
    ...
    const timerOptionElement = <div class="join">
    <button class="btn join-item btn-timer" >15</button>
    <button class="btn join-item btn-timer">30</button>
    <button class="btn join-item btn-timer">60</button>
    <button class="btn join-item btn-timer">120</button>
    </div>

    const wordOptionElement = <div class="join">
    <button class={`btn join-item btn-timer ${props.childBtnColor[0]}`} value='10' onClick={(event)=>generatePhraseBasedOnWordCount(event)} >10</button>
    <button class={`btn join-item btn-timer ${props.childBtnColor[1]}`} value='25' onClick={(event)=>generatePhraseBasedOnWordCount(event)}>25</button>
    <button class={`btn join-item btn-timer ${props.childBtnColor[2]}`} value='50' onClick={(event)=>generatePhraseBasedOnWordCount(event)}>50</button>
    <button class={`btn join-item btn-timer ${props.childBtnColor[3]}`} value='100' onClick={(event)=>generatePhraseBasedOnWordCount(event)}>100</button>
    </div>
    ...
    return (
        <div class="join">
            ...
            <button class={`btn join-item btn-feature ${wordbtnColor}`} onClick={(event)=>{updateWordBtnStyle(event)}}>
